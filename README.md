...
Dicas com mais ferramentas principalmente para desenvolvimento REACT no dia-a-dia:
...
1. **`React Context API`**
    ...
    - Corrige muito bem o problema de prop drilling: quando você passa a mesma propriedade para muitos components filhos;
    
2. **`Redux`**
    - Cria um estado global, que será acessado por toda a aplicação;
    - Lida muito bem com os estados muito complexos e side effects (quando um evento ocorre por causa de uma ação do usuário ou própria aplicação);

**`React Context API`** ou **`Redux`**

|Context API|Redux|
|---|---|
|Contextos Específicos| Contexto Global|
|É um hook do próprio React.js|É um pacote a parte, podendo ser usado com Angular, Vue, etc.|
|Lida somente com Estado|Lida com Estado e Site Effect|
...
3. **`Gatsby.js`**
    ...
    - Tem todo ecossistema já preparado para garantir o máximo de produtividade pro desenvolvedor;
